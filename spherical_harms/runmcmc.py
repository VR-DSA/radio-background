...
aguess = [0.05, 2.*d, 0.3*d, 2.5e-41, 4.*d, 4.e-42] # this matches the data well
nsid = NSIDE_sky
...
def lnprior(param):
    ...
    if d < R_disk < 5.*d and 0.00001*d < h_disk < 5.*d and 1e-44 < j_disk < 1e-37 and d < R_halo < 5.*d and 1e-44 < j_halo < 1e-37:
    #if d < R_disk < 5.*d and 0.00001*d < h_disk < 5.*d and 1e-44 < j_disk < 1e-37:
        return 0.0
...
def lnprob(param, d1, d2):
    ...
    if not np.isfinite(lp):
        return -np.inf
    lpp = lp + LP.lik_disk_halo(param, (d1, d2))
    if np.isnan(lpp):
        return -np.inf
    return lpp
...
for i in range(nwalkers-1):
    ...
    vary = np.abs(np.random.randn(ndim)/100.+1)
...
### output results ###
# You change: which elements are scaled by d, and output file name
samples = np.array(sampler.chain)
samples[:,:,1:3] /= d
samples[:,:,4] /= d
print('Average acceptance fraction: ', np.mean(sampler.acceptance_fraction))
np.savez("results/output.npz", samples=samples)
